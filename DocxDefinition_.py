...
def createHeader(cell, content_obj, no=None, depth=2):
    ...
    if hj:
        parse_json_content(cell, hj, header = True, no = no, tier = depth)
    elif no:
        p = cell.add_paragraph()
...
def createTable(cell, content_obj):
    """ct=2 單表渲染（table_json 內含一個 tiptap table）。參數表(ct=4)走 create_parameter_table，不經此。"""
    if isinstance(content_obj.get("table_json"), dict):
...
def _render_node_dfs(cell, chapter, index_path, node):
    """
    一般階層節點 DFS 渲染（L2..L8）。順序固定：標題 → 自身內容 → children（spec §19 F4）。
    編號用 format_node_number(chapter, index_path)。option 沿用舊 content_type：
      0/3 純標題、1 標題+文字+圖、2 標題+表格。
    """
    depth = 1 + len(index_path)
    no = format_node_number(chapter, index_path)
    option = node.get("content_type")

    if option == 2:  # 標題 + 表格（包進單格表格做 keep-together，沿用舊佈局）
        ...
        createHeader(target_cell, node, no=no, depth=depth)
        createTable(target_cell, node)
        createContent(target_cell, node, depth=depth)
        all_paragraphs = target_cell.paragraphs
        ...
        createHeader(target_cell, node, no=no, depth=depth)
        createContent(target_cell, node, depth=depth)
        createPictures(target_cell, node)
    else:  # option 0 / 3：純標題
...
def fill_from_template(template_path, out_path, data, title_mapping, info_mapping, styled_mapping=None):
    ...
    enable_docx_protection(doc, "1q2w3e4RR")
...
def get_docx_(outpath, data, template = "docx-template/example3.docx"):
    ...
    for index, attr in enumerate(data["attribute"]):
        info_mapping[f"REV{index + 1}"] = f'{attr["document_version"]:.1f}'
